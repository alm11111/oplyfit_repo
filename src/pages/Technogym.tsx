...
interface MywellnessSyncResult {
...
export default function Technogym() {
  const { data: statusData } = useQuery({
    queryKey: ['technogym', 'status'],
    queryFn: () => api.get<StatusDto>('/api/v1/technogym/status'),
  })

  const { data: equipmentData, isLoading, refetch } = useQuery({
    queryKey: ['technogym', 'equipment'],
    queryFn: () => api.get<MywellnessEquipment[]>('/api/v1/technogym/equipment'),
  })

  const syncMutation = useMutation({
    mutationFn: () => api.post<MywellnessSyncResult>('/api/v1/technogym/sync', {}),
    onSuccess: () => refetch(),
  })
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-ink">Technogym Mywellness</h1>
        <p className="mt-1 text-sm text-slate-500">
          Integrazione OAuth2 con l'ecosistema Mywellness — attrezzature e sessioni di allenamento.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="font-semibold text-ink">Mywellness API</div>
            <div className="text-sm text-slate-500">OAuth2 Client Credentials</div>
          </div>
      ...
      <div className="flex items-center justify-between">
        ...
        >
          {syncMutation.isPending ? 'Sincronizzazione…' : '🔄 Sincronizza sessioni'}
        </button>
      </div>

      {syncResult && (
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-700">
          Sync completato: {syncResult.equipmentSynced} attrezzature, {syncResult.sessionsSynced} sessioni
        </div>
      )}
              ...
              <tbody className="divide-y divide-slate-50">
                {equipment.map(eq => (
                  <tr key={eq.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-slate-600">{eq.brand}</td>
                    ...
                    <td className="px-4 py-3 text-slate-400 text-xs">
                      {eq.lastSyncedAt ? new Date(eq.lastSyncedAt).toLocaleString('it-IT') : '—'}
                    </td>
                  </tr>
                ))}
                {equipment.length === 0 && (
